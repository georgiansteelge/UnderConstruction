...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.blog-card').forEach(card => {
    const slides = card.querySelectorAll('.blog-slide');
    const dots   = card.querySelectorAll('.bdot');
    const prevBtn = card.querySelector('.blog-prev');
    const nextBtn = card.querySelector('.blog-next');
    let current  = 0;
    let autoTimer;
    ...
    function startAuto() { autoTimer = setInterval(() => showSlide(current + 1), 4000); }
    function stopAuto()  { clearInterval(autoTimer); }

    nextBtn?.addEventListener('click', () => { stopAuto(); showSlide(current + 1); startAuto(); });
    prevBtn?.addEventListener('click', () => { stopAuto(); showSlide(current - 1); startAuto(); });

    dots.forEach(dot => {
      dot.addEventListener('click', () => {
        stopAuto();
        showSlide(parseInt(dot.dataset.b));
        startAuto();
      });
    });
